...
def role_security(
    requiredRole: Role = viewer_role, user: User = Depends(get_current_user)
) -> User:
...
async def login(creds: OAuth2PasswordRequestForm = Depends()) -> Token:
    for user in UserDB:
        if (
            ...
            and user.get("hash") == hashlib.sha256(creds.password.encode()).hexdigest()
        ):
            payload = {
...
async def about_me(
    user: User = Depends(partial(role_security, requiredRole=viewer_role))
) -> dict:
...
@app.get(path="/admin")
async def admin(
    user: User = Depends(partial(role_security, requiredRole=admin_role))
) -> dict:
...
def async_side_effect(
    ...
    **kwargs_to_side_func,
    ...
    async def inner():
        ...
        if not is_successful(result_self):
            return result_self
        result_side_effect = await future_safe(func)(
            *args_to_side_func, **kwargs_to_side_func
        )
        if not is_successful(result_side_effect):
            return result_side_effect
        return result_self._inner_value
...
def bind_side_effect(
    ...
    **kwargs_to_side_func,
    ...
    async def inner():
        result_side_effect = await self.bind_result(
            lambda value: safe(func)(value, *args_to_side_func, **kwargs_to_side_func)
        )
        if not is_successful(result_side_effect):
...
def bind_async_side_effect(
    ...
    **kwargs_to_side_func,
    ...
    async def inner():
        result_side_effect = await self.bind(
            lambda value: future_safe(func)(
                value, *args_to_side_func, **kwargs_to_side_func
            )
        )
        if not is_successful(result_side_effect):
            return result_side_effect._inner_value
...
@app.get(
    "/todo/{id}",
    dependencies=[Depends(partial(role_security, requiredRole=viewer_role))],
)
@ResultWrapper
async def todo_get(itemid: int):
    async with session:
        return await future_safe(session.get)(
            entity=Todo, ident=itemid
        ).assert_not_none(
...
@app.post(
    path="/todo",
    dependencies=[Depends(partial(role_security, requiredRole=developer_role))],
)
@ResultWrapper
async def todo_post(item: Item):
    async with session:
        ...
        return (
            await FutureResult.from_value(todo)
...
async def todo_put(todo: Todo):
    async with session:
        return (
            await future_safe(session.get)(entity=Todo, ident=todo.itemid)
            .assert_not_none(
                HTTPException(
            ...
            .bind_side_effect(
                lambda x: x.sqlmodel_update(todo.model_dump(exclude_unset=True))
            )
            .bind_side_effect(session.add)
            .async_side_effect(session.commit)
            .bind_async_side_effect(session.refresh)
...
@app.delete(
    path="/todo",
    dependencies=[Depends(partial(role_security, requiredRole=admin_role))],
)
@ResultWrapper
async def todo_delete(todo_id: int):
    async with session:
        return (
            await future_safe(session.get)(entity=Todo, ident=todo_id)
            .assert_not_none(
                HTTPException(
                    status_code=status.HTTP_404_NOT_FOUND, detail="Not found!"
                )
